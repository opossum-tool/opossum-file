...
from opossum_lib.input_formats.owasp_deependency_scan.services.owasp_dependency_scan_file_reader import (
...
def generate(
    ...
    total_number_of_files = len(scancode_json_files) + len(opossum_files) + len(owasp_dependency_scan_files)
    ...
    input_readers += [OwaspDependencyScanFileReader(path=path)
                      for path in owasp_dependency_scan_files]
